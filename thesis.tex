...
W~sekcji \textit{Budowa typowego fragmentu} zwróciliśmy uwagę na identyczny cykl życia
...
jako osobny proces, a~fragmenty mogą współdzielić dane i~struktury, kod widżetu jest
...
ale system ma prawo ją zignorować ze względów np. oszczędzania energii.
...
oraz przycisku wymuszonego odświeżenia planu. Przykładowy rysunek można znaleźć w~rozdziale
...
Potrzebna jest więc możliwość przekazania adapterowi przyczyny rozkazu odświeżenia widoku --
czy z~powodu zmiany rozwinięcia elementu, zmiany dnia, czy też na skutek rozkazu odświeżenia
...
\begin{figure}[h]
	...
	\begin{subfigure}[h]{0.4\textwidth}
		...
		\caption{Nazwa aplikacji i loga}
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{Oceny}\label{fig:grades}
	\medskip
\end{figure}
...
protokoły do całych zajęć, wykładu i ćwiczeń (przykład na Rysunku \ref{fig:grades_complicated}). Student otrzymuje informacje o ocenie
i wystawiającym. Miłym dodatkiem do załączonych informacji jest wykres z ocenami
wszystkich studentów (\ref{fig:grades_expanded}). Ponadto funkcjonuje system powiadomień który sprawia, że
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{Protokoły}\label{fig:protocols}
	\medskip
\end{figure}

Protokoły służą do wystawiania ocen przez prowadzącego. Po otwarciu modułu wykładowca
widzi listę protokołów (Rysunek \ref{fig:protocols_list}), która jest przeszukiwalna. Po kliknięciu protokołu i wybraniu
sesji egzaminacyjnej, prowadzący uzyskuje dostęp do listy studentów i ich ocen (\ref{fig:protocols_people_list}). W tym
momencie może sprawdzić informacje o protokole, statystyki ocen albo wyszukać
studenta i wystawić mu ocenę (\ref{fig:protocols_edit}). Pierwsze wystawienie oceny musi być potwierdzone przez
wizytę na ekranie logowania (\ref{fig:protocols_access}). Ze względów bezpieczeństwa informacje o studentach nie
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{Sprawdziany}\label{fig:tests}
	\medskip
\end{figure}
...
Po wybraniu konkretnego sprawdzianu otwiera się widok jego korzenia (\ref{fig:tests_root}). Jako pierwsza
zawsze jest wyświetlana karteczka z~właściwościami i~statystykami aktualnie otwartego
węzła (\ref{fig:tests_root_details}). Następnie pokazywane są karteczki reprezentujące dzieci danego węzła.
W zależności od typu dziecka, można wejść w głąb drzewa (\ref{fig:tests_folder}) lub rozwinąć informację o~punktach i~ocenie (\ref{fig:tests_details}).
...
analogiczny do protokołów (\ref{fig:emptests_grading}). Pewnym dodatkiem, który nie występował w wymienionych
...
karteczce, która wyświetla właściwości właśnie otwartego węzła (\ref{fig:emptests_hide}).

\section{Plan zajęć}

\begin{figure}[h]
	\centering
	\includegraphics[width=0.3\textwidth]{img/timetable.png}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.3\textwidth]{img/calendar_widget.png}
...
Kalendarz roku akademickiego (Rysunek \ref{fig:calendar}) dostarcza informacji
o~najważniejszych wydarzeniach w~roku akademickim, takich jak okresy wolne, podpięcia czy
sesja. Zdarzenia są wizualizowane przy użyciu kolorowych ikon, a~po wybraniu dnia pod
kalendarzem można poznać szczegółowe opisy wydarzeń (\ref{fig:calendar_item}).
...
Student może kliknąć wybraną grupę aby zobaczyć szczegóły wybranej grupy zajęciowej (\ref{fig:groups_group}). W szczególności
...
W tym widoku (Rysunek \ref{fig:course}) student może sprawdzić syllabus przedmiotu, różne informacje wprowadzone
...
\begin{figure}[h]
	...
	\end{subfigure}
	\caption{Ankiety}\label{fig:surveys}
	\medskip
\end{figure}
...
zajęciach i wykładowcy (\ref{fig:surveys_survey}).
...
API w celu dostarczenia do wybranych osób (\ref{fig:usosmail_compose}).
...
Moduł ,,Moje legitymacje'' (Rysynek \ref{fig:idcards}) pozwala na wyświetlenie podglądu wszystkich legitymacji
...
,,Moje eID'' (Rysunek \ref{fig:myeid}) wyświetla numery identyfikacyjne przypisane do użytkownika takie jak
...
tytuł, nagłówek i treść artykułu (\ref{fig:news_item}). Możliwa jest łatwa nawigacja gestami pomiędzy
artykułami.
...
może otworzyć nawigację (\ref{fig:map_item}).
...
Na tym ekranie (Rysunek \ref{fig:settings}) użytkownik może ustawić:
